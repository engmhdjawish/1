...
final class WebCustomerService
{
    ...
    public static function registerSelf(string $name, string $phone, string $password, ?string $email = null): array
    {
        ...
        try {
            NotificationService::notifyCustomerRegistrationPending($customerId);
        } catch (\Throwable) {
        }
        ...
        return [
            'ok' => true,
            'message' => 'تم التسجيل. ستصلك إشعار عند تفعيل حسابك — يمكنك التصفح بصلاحيات الزائر حتى ذلك الحين.',
            'id' => $customerId,
