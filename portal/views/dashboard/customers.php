...
<section class="mb-5 flex flex-col md:flex-row gap-3">
  <form method="get" class="flex-1 grid grid-cols-1 sm:grid-cols-[1fr_auto_auto] gap-2">
    <input type="hidden" name="status" value="<?= h($statusFilter) ?>">
    <input name="q" value="<?= h($searchFilter) ?>" class="h-11 rounded-xl border border-border-subtle px-4" placeholder="بحث بالاسم، الهاتف، أو البريد...">
    <select name="source" class="h-11 rounded-xl border border-border-subtle px-3 text-sm">
    ...
    <button class="h-11 rounded-xl bg-slate-900 text-white px-5 font-bold">بحث</button>
  </form>
</section>
                ...
                <div class="flex flex-wrap items-center justify-end gap-1.5">
                  ...
                  <?php if ($canApproveCustomers && $status === 'pending'): ?>
                    <form method="post" class="inline-flex items-center gap-1">
                      ...
                      <button name="action" value="approve" class="h-8 px-2 rounded-lg bg-emerald-600 text-white text-[11px] font-bold">موافقة</button>
                      <button name="action" value="reject" class="h-8 px-2 rounded-lg bg-red-600 text-white text-[11px] font-bold">رفض</button>
                    </form>
                  <?php endif; ?>

                  <?php if ($canManageCustomers && in_array($status, ['active', 'pending'], true)): ?>
                    <form method="post" class="inline" onsubmit="return confirm('تعليق هذا الحساب؟ لن يتمكن العميل من الدخول.');">
                      <input type="hidden" name="action" value="suspend">
                      <input type="hidden" name="customer_id" value="<?= h($rowId) ?>">
                      <button class="h-8 px-2.5 rounded-lg bg-amber-500 text-white text-[11px] font-bold hover:bg-amber-600">تعليق</button>
                    </form>
                  <?php endif; ?>
                  ...
                  <?php if ($canApproveCustomers && in_array($status, ['suspended', 'rejected'], true)): ?>
                    <form method="post" class="inline-flex items-center gap-1">
                      ...
                      <button class="h-8 px-2 rounded-lg bg-primary text-white text-[11px] font-bold">تفعيل</button>
                    </form>
                  <?php endif; ?>
                  ...
                  <?php if ($canManageCustomers && $status === 'active'): ?>
                    <form method="post" class="inline">
                      ...
                      <button class="h-8 px-2.5 rounded-lg border text-[11px] font-bold <?= $isActive ? 'border-slate-300 text-slate-600' : 'border-emerald-300 text-emerald-700' ?>">
    ...
    <?php else: ?>
      <form method="post" class="p-5 space-y-4">
        ...
        <div class="flex flex-wrap gap-2 pt-1">
          <button type="submit" class="flex-1 min-w-[140px] h-11 rounded-xl bg-primary text-white font-bold hover:brightness-110">
          ...
          <?php if ($editing): ?>
            ...
            <?php if (in_array($editStatus, ['active', 'pending'], true)): ?>
              <button
                ...
                name="action"
                value="suspend"
                class="h-11 px-4 rounded-xl bg-amber-500 text-white font-bold text-sm"
