...
/** @var array<string, list<array<string, mixed>>> $siteContentGroups */
?>
<section class="mb-6">
  <div class="flex flex-wrap items-start justify-between gap-3">
    <div>
      <h1 class="text-2xl font-extrabold text-slate-900">محتوى الموقع</h1>
      <p class="text-sm text-text-muted mt-1 max-w-2xl">
        إدارة ما يراه زوار الموقع: الصفحة الرئيسية، العروض، الوسائط، وصفحة من نحن.
        لإعدادات المستخدمين والربط التقني انتقل إلى
        <a href="/dashboard/configuration.php" class="text-primary font-bold hover:underline">إدارة النظام</a>.
      </p>
    </div>
...
<?php foreach ($siteContentGroups as $groupTitle => $items): ?>
  <section class="mb-8">
      ...
      <?php foreach ($items as $item): ?>
        ...
        >
          <div class="flex items-center gap-3">
            <span class="w-11 h-11 rounded-xl bg-primary/10 text-primary flex items-center justify-center group-hover:bg-primary transition group-hover:text-white">
              <span class="material-symbols-outlined"><?= h((string) ($item['icon'] ?? 'web')) ?></span>
            </span>
            <div>
              <h3 class="font-bold text-slate-900"><?= h((string) ($item['label'] ?? '')) ?></h3>
              <p class="text-xs text-text-muted mt-0.5">محتوى الموقع</p>
            </div>
          </div>
